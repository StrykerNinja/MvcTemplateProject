        ...
        private static void AddParameterValuesFromExpressionToDictionary(RouteValueDictionary rvd, MethodCallExpression call)
        {
            ...
            if (parameters.Length > 0)
            {
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    {
                        new RouteValueDictionary(value).ToList().ForEach(x =>
                        {
                            var variables = x.Value as IEnumerable;
                            if (variables != null && !(x.Value is string))
                            {
                                var index = 0;
                                foreach (var variable in variables)
                                {
                                    rvd.Add(string.Format("{0}[{1}]", x.Key, index), variable);
                                    index++;
                                }
                            }
                            else
